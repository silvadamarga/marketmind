...
def macro_monitor_loop():
    ...
    while True:
        ...
        # News-vol posture: refresh + alert on a regime-level change (shadow /
        # decision-support only — never gates anything). Lazy import avoids a
        # circular dependency (market_posture reads VIX from this module's cache).
        try:
            import market_posture
            from notifications import send_posture_alert
            vix, _ = get_market_regime_from_cache()
            change = market_posture.check_regime_change(vix=vix or None)
            if change:
                old, new, posture = change
                print(f"📊 News-vol regime change: {old} → {new} (score {posture['score']})")
                send_posture_alert(old, new, posture)
        except Exception as e:
            print(f"⚠️ Posture check error: {e}")
