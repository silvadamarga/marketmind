...
export default function DailyAnalysis() {
    ...
    const { total_events, sentiment_counts, top_tickers, top_categories, priority_events, thresholds } = data;
    ...
    return (
        <div className="flex-1 overflow-y-auto p-4 sm:p-8 scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent">
            <div className="max-w-6xl mx-auto space-y-8">
                ...
                {/* Header */}
                <div>
                    <h1 className="text-3xl font-bold text-white mb-2">Daily Market Pulse</h1>
                    <p className="text-slate-400">The last 24 hours of market intelligence, by the numbers.</p>
                </div>
                ...
                <div>
                    ...
                    {thresholds && (
                        <p className="text-xs text-slate-500 -mt-2 mb-4">Impact ≥ {thresholds.impact} or novelty ≥ {thresholds.novelty}</p>
                    )}
                    <div className="space-y-4">
                        {priority_events.length === 0 ? (
                            <div className="text-slate-500 italic">No priority events in the last 24 hours.</div>
                        ) : (
                            priority_events.map((event) => (
                                <div key={event.id} className="bg-[#131b2e] border border-slate-800 p-5 rounded-xl hover:border-slate-600 transition-colors">
                                    <div className="flex justify-between items-start mb-2">
                                        <div className="flex items-center space-x-2">
                                            ...
                                            <span className="text-xs text-slate-500">{new Date(event.date).toLocaleString(undefined, { weekday: 'short', hour: '2-digit', minute: '2-digit' })}</span>
                                        </div>
